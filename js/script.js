...
/* Constantes */
const C = 0, // Cantidad de polígonos
  N = 1,     // Nombre
  B = 2;     // Color de fondo (RGB)
...
function applySet(name) {
  ...
  tris.forEach((tri, index) => {
    ...
    if (i <= count) {
      ...
      tri.style.transform = `translate(${randX}px, ${randY}px) scale(0) rotate(${randRot}deg)`;
...
autoBtn.addEventListener("click", () => {
  ...
  if (isPlaying) {
    ...
    // (Opcional: si prefieres que no haga nada, quita este if)
    if (Math.ceil(window.scrollY) >= Math.floor(document.body.scrollHeight - window.innerHeight)) {
        window.scrollTo({ top: 0, behavior: "smooth" });
        autoIndex = 0;
    } else {
        // Si estamos a mitad, calculamos el índice actual
...
function onScroll() {
  ...
  // Si estamos en el último animal (Colibrí)
  if (index === keys.length - 1) {
    // Esperamos 2 segundos viendo al colibrí antes de lanzar los textos
    setTimeout(() => {
        // Verificamos que sigamos en el final (por si el usuario subió rápido)
...
function startExperience() {
  ...
  // Esto hace que los triángulos "entren" justo cuando la pantalla negra se desvanece
  setTimeout(() => {
    applySet(keys[0]);
  }, 500);
}
...
/* NOTA: Eliminamos la línea antigua: setTimeout(() => applySet(keys[0]), 100); */
/* Ahora la animación no empieza hasta que tú lo digas. */
